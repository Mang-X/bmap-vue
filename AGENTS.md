...
| `pnpm check:api` | **三类基线**无漂移（需先 `pnpm build:package`）：① 5 份 API report（`packages/bmap-vue/etc/<出口>/bmap-vue.api.md`，出口 = `advanced` / `composables` / `plugins` / `resolver` / `ui-kit`）；② 5 份**未导出类型身份集合**（`etc/<出口>/forgotten-exports.json`，`ae-forgotten-export` 的符号名**全等**比对——新增与清理两个方向都红，只比条数会漏掉 1-for-1 替换与跨提交回弹）；③ 2 份**类型级签名基线**（`etc/{index,components}/bmap-vue.dts.md`，`dist` 声明经 TS printer `removeComments` 规范化）。`.` 与 `./components` 进不了 API report（Volar `__VLS_` 悬空引用），按**探针**断言该阻塞仍在。改了公共类型面就跑 `pnpm generate:api` 并把基线一起提交 |

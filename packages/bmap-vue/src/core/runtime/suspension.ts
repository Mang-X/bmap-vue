 ...
 *
 * 「暂停原因」是 `MapRuntime` 与 `<Map>` / `useMapSuspension` 之间的共同词汇，因此和
 ...
 * | 原因 | 谁加的 | 谁移除 | 说明 |
 * | --- | --- | --- | --- |
 * | `user` | `MapExpose.suspend()`（默认原因） | 调用方 `resume()` | 业务主动暂停；**优先级最高**（页面可见性变化不得解除它） |
 * | `keep-alive` | `<Map>` 的 `onDeactivated` | `onActivated` | KeepAlive 停用（不销毁 WebGL 地图） |

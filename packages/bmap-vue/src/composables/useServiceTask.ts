/**
 * useServiceTask —— 服务类 composable 的统一状态机（M7-SERVICE-CORE / issue #38）
 *
...
export interface UseServiceTaskOptions<
  TDriver,
  THandle,
...
export interface ServiceTask<TResult, TArgs extends unknown[], THandle = unknown> {
...
export function useServiceTask<TDriver, THandle, TArgs extends unknown[], TResult = TDriver>(
  ctx: MapContext,
  options: UseServiceTaskOptions<TDriver, THandle, TArgs, TResult>,
): ServiceTask<TResult, TArgs, THandle> {
  ...
  /** 当前 Client（`<Map>` 子树里通常已经就绪；Provider-only 场景要等 `whenReady`）。 */
  ...
  const tryRelease = (entry: CachedService): boolean => {
    ...
    try {
      ...
      logger.warn(
        `useServiceTask: 释放 ${options.capability} 的实例失败，已保留引用待下一次释放重试（` +

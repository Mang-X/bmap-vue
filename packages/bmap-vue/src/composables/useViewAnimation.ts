...
export function useViewAnimation(
  ...
  /** 传入 <Map> 组件实例 ref 时解包出真正的 MapHandle */
  ...
  function cancel(): void {
    ...
    if (failures.length > 1) {
      ...
      logger.warn(
        `useViewAnimation: 一次 cancel() 里有 ${failures.length} 条取消失败，` +
  ...
  onUnmounted(() => {
    ...
    for (const run of runs) {
      try {
        ...
        logger.warn(
          "useViewAnimation: 卸载时取消视角动画失败，已交由地图销毁重试: " +

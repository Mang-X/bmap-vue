/**
 * useGeocodeDetail（M7-SERVICE-CORE / #38）
 *
...
import { useGeocodeDetail } from "./useGeocodeDetail";
...
function mountWithHook(
  ctx: MapContext,
  run: (hook: ReturnType<typeof useGeocodeDetail>) => void | Promise<void>,
) {
  const Child = defineComponent({
    setup() {
      const hook = useGeocodeDetail();
...
describe("useGeocodeDetail", () => {
  ...
  it("getBatch keeps per-item results（部分成功：每项带自己的终态）", async () => {
    ...
    let batch: Awaited<ReturnType<ReturnType<typeof useGeocodeDetail>["getBatch"]>> | null = null;
    const wrapper = mountWithHook(ctx, async (hook) => {

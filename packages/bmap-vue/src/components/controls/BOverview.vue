...
export interface BOverviewProps {
  anchor?: string;
...
/**
 * BOverview —— 缩略地图控件 / 鹰眼（官方 `OverviewMapControl`）
 *
 ...
 */
const props = withDefaults(defineProps<BOverviewProps>(), {
  anchor: "BMAP_ANCHOR_BOTTOM_RIGHT",
  offset: () => ({ x: 0, y: 0 }),
  // 官方声明的默认值（`@default false`）：显式写出，避免 Vue 的布尔转换让「用户显式传
  // false」与「用户没传」不可区分（见 `BMapType` 的同一条注释）。
  isOpen: false,
  visible: true,
});

const spec: ControlSpec<BOverviewProps> = {
  kind: "overview",
  options: (p) => ({
    anchor: p.anchor,
    offset: p.offset,

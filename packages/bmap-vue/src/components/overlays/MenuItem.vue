...
/**
 * MenuItem —— 声明式菜单项（M5-CUSTOM-MENU / issue #33）
 *
 * ```vue
 * <ContextMenu>
 *   <MenuItem text="标记此处" @select="onSelect" />
 *   <MenuSeparator />
 ...
 * ## 与数据 API 的关系
 *
 * `<MenuItem text="A" />` 与 `items: [{ text: "A" }]` 产出的条目**完全同形**（顺序、`disabled`、
...
import type { MenuItemProps, ContextMenuSelectPayload } from "../../types/components";
...
if (registry) {
  ...
  // 放错位置（不在 <ContextMenu> 子树里）：**明确告警**，而不是静默变成一个永不出现的菜单项
  logger.warn(
    "MenuItem: 没有找到父级 <ContextMenu>；这一项不会被加入任何菜单。" +
      "声明式菜单项必须放在 <ContextMenu> 的子节点里。",
  );
}

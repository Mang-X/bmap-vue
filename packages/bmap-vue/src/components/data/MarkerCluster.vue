...
/**
 * MarkerCluster —— 空间聚合（M6-POINT-CLUSTER / issue #35）
 *
 ...
 * | 命中 | 事件 | 载荷 |
 * | --- | --- | --- |
 * | 簇 | `cluster-click` | `ClusterPick<Item>`：`engine` / `id` / `size` / `position` / `items` |
...
import type { ClusterChange, ClusterPick, MarkerClusterProps } from "../../types/components";
import type { MapReadyContext } from "../../core/context/types";

const props = withDefaults(defineProps<MarkerClusterProps<Item>>(), {
  // 布尔 / 字符串 / 数字三类都要显式表态：
...
const emit = defineEmits<{
  ...
  "cluster-click": [pick: ClusterPick<Item>];
  /** 聚合结果读数（两个引擎同名同形：`{ engine, clusters, singles, zoom }`）。 */
  "cluster-change": [change: ClusterChange];
  /** 未聚合的单点被点击（载荷是**最新**的业务 item）。 */
  "item-click": [item: Item];
}>();

const ctx = useRequiredMapContext();
const scope = new ResourceScope({ label: "MarkerCluster" });
...
function warnOptionMismatch(): void {
  ...
  for (const [name, value] of mismatched) {
    ...
    devWarn(
      `[MarkerCluster] ${String(name)} 只对 engine="${props.engine === "native" ? "markers" : "native"}" 生效，` +
...
function createEngine(c: MapReadyContext): ClusterEngine<Item> {
  const shared = {
    ready: c,
    label: "MarkerCluster",
    onProblem: (message: string) => devWarn(`[MarkerCluster] ${message}`),
...
function reportError(error: unknown): void {
  ...
  ctx.events.emit("resource:error", { error: wrapped, component: "MarkerCluster" });
}

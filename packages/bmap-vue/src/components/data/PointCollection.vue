...
/**
 * PointCollection —— 批量点（**单个** SDK 资源，M6-MARKER-POINTCOLLECTION / issue #34）
 *
 * ## 为什么不是「一个点一个 Marker」
 *
 ...
 */
const reports = createProblemReporter("PointCollection", (message) => devWarn(message));
...
function ctorOptions(p: Readonly<PointCollectionProps<Item>>): Record<string, unknown> {
  return {
...
const resource = useNativeLayerResource<PointCollectionProps<Item>>(props, {
  ...
  // PointCollection 的要素状态命令面**不会**遇到「身份未声明」那条拒绝路径。
...
function handlePick(event: unknown): void {
  ...
  if (pick.item !== null) emit("item-click", pick.item);
    ...
    warnOnce(
      "pick-unresolved",
      `[PointCollection] 命中了要素（dataIndex=${pick.dataIndex}）但取不到业务项：` +

...
import type { MapExpose } from "../../types/mapExpose";
...
// 「页面已有全局就自动回退」。它们走的是 legacy `existingGlobalProvider()`（读 `Map ?? BMapGL`），
...
function disposedError(): BMapError {
  return new BMapError(
    ...
    "Map map runtime disposed while a retry was pending",
  );
}
 ...
 *
 * `<Map>` 声明 Catalog 里的全部 map 事件（模板 / TS 都有完整提示），并在**地图就绪时一次性**
 ...
 * 于是「监听器从 `undefined` 变成函数」这种变化**不会让 `<Map>` 重渲染**，依赖 `onUpdated` 的
 ...
 *
 * 返回类型**显式标注**为 `MapExpose`：`defineExpose()` 会把它推导成组件实例类型，因此消费方

...
/**
 * TrackLineLayer —— 原生轨迹线图层（官方 4.0 扩展 API `TrackLine`）
 *
 * ## 能力面
 *
...
const props = withDefaults(defineProps<TrackLineLayerProps>(), {
  visible: true,
  pauseOnHidden: false,
});
...
const emit = defineEmits<{
  ...
  progress: [observed: TrackLineObserved];
  /** `statuschange` 事件：SDK 播放状态变化时派发（载荷见 `observed`）。 */
  statuschange: [observed: TrackLineObserved];
}>();
 ...
 */
const observed = shallowRef<TrackLineObserved | null>(null);
...
const { resource } = useVisualLayer<TrackLineLayerProps>(props, {
  kind: "track-line",
 ...
 * 解包，接口类型因此是**值**（`TrackLineObserved | null`），与消费方实际读到的一致。
 */
function createExpose(): TrackLineLayerExpose {
  return {
    playback,
    get observed() {
...
defineOptions({ name: "TrackLineLayer" });
</script>

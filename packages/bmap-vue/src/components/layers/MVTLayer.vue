...
/**
 * MVTLayer —— MVT 矢量瓦片图层（官方 `BMap.MVTLayer`，4.0）
 *
...
const emit = defineEmits<{
  click: [e: MVTLayerPickEvent];
  dblclick: [e: MVTLayerPickEvent];
  mousemove: [e: MVTLayerMouseMoveEvent];
  mouseout: [e: MVTLayerMouseEvent];
  tilesloadstart: [e: MVTLayerBaseEvent];
  tilesloadend: [e: MVTLayerBaseEvent];
}>();

const resource = useLayerResource<MVTLayerProps>(props, {
  component: "MVTLayer",
  toSpec: (p) => ({
    kind: "mvt",
    visible: p.visible,
  ...
  bind: ({ handle, context, scope }) => {
    ...
    scope.add(events.on(handle, "click", (e) => emit("click", e as MVTLayerPickEvent)));
    scope.add(events.on(handle, "dblclick", (e) => emit("dblclick", e as MVTLayerPickEvent)));
    scope.add(
      events.on(handle, "mousemove", (e) => emit("mousemove", e as MVTLayerMouseMoveEvent)),
    );
    scope.add(events.on(handle, "mouseout", (e) => emit("mouseout", e as MVTLayerMouseEvent)));
    scope.add(
      events.on(handle, "tilesloadstart", (e) =>
        emit("tilesloadstart", e as MVTLayerBaseEvent),
      ),
    );
    scope.add(
      events.on(handle, "tilesloadend", (e) => emit("tilesloadend", e as MVTLayerBaseEvent)),
    );
  },
});

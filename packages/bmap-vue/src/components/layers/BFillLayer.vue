...
/**
 * BFillLayer —— 原生批量面图层（官方 `BMap.FillLayer`，4.0）
 *
 * 与 `BLineLayer` 同构（同一份装配 `useVisualLayer`），差别只有三处：
 *
 * 1. 图层种类是 `fill`（官方 `FillLayer`）；
 * 2. 样式是 `BFillLayerStyle`（官方 `FillLayerStyle`：纯色 / 描边 / 纹理三套）；
 ...
 * 构造期项换实例 / 拾取载荷与未命中语义）与 `BLineLayer` 完全一致。
 */
import { NATIVE_LAYER_PICK_EVENTS, pickEmitterFor, useVisualLayer } from "./useVisualLayer";
import type { BFillLayerProps, BMapFeaturePick } from "../../types/components";

const props = withDefaults(defineProps<BFillLayerProps>(), {
...
const { resource } = useVisualLayer<BFillLayerProps>(props, {
  kind: "fill",

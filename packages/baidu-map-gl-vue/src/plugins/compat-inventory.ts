...
export interface PluginCompatEntry {
   ...
   * 它记的是**最小路径**的结果，不承诺整条功能链路（「构造成功」不等于「画完一个多边形」）。
   */
  readonly runtime?: {
...
export const PLUGIN_COMPAT_INVENTORY: readonly PluginCompatEntry[] = [
  {
    ...
    residualRisks: [
      ...
      "只跑通最小路径：`pause` / `continue` / `setSpeed` / 播放到结尾这些分支未覆盖；" +
        "把它们搬进 CI / nightly 属 #43。",
    ],
  },
  {
    id: "DrawingManager",
    ...
    runtime: {
      status: "verified",
      detail:
        "真实 4.0 页面上 `new BMapGLLib.DrawingManager(map, { isOpen: false, enableCalculate: true, " +
        "enableGpc: true })` 构造成功，`getDrawingMode()` 返回 `marker`；**证实了「脚本自行注入外部脚本」**" +
        "——页面上多出 `GeoUtils/src/GeoUtils.min.js` 与 `DrawingManager/src/gpc.js` 两个 `<script>`" +
    ...
    residualRisks: [
      "`new BMapGL.Overlay` 与官方「不可实例化」的表述冲突；**真实 4.0 上构造成功、未抛错**，" +
        "但「能继承基类」≠「绘制交互可用」——实际画一个圆/多边形的路径未覆盖，归 #43。",
  ...
  },
  {
    ...
    runtime: {
      ...
      detail:
        ...
        "111194.87（≈1° 纬度，符合预期）。注：三参形态的 `isPointInRect(point, sw, ne)` 返回 `false` " +
        "且不抛错——该形态的语义未确认，本库不据此下结论。",
    },
    summary:
    ...
    residualRisks: [
      ...
      "`DrawingManager` 会自行注入它，属于插件之间的隐式依赖（运行时已观察到）。" +
        "各静态谓词的**签名语义**未逐个核对（只证了不抛错 + `getDistance` 数值正确）。",
    ],
  },
  {
    id: "Mapvgl",
    ...
    runtime: {
      status: "threw",
      detail:
        "真实 4.0 页面上 `window.mapvgl` 与 `View` 都在，但 `new mapvgl.View({ map, mapType: \"bmap\" })` " +
        "抛 `Cannot read properties of undefined (reading 'appendChild')` —— 它的视图容器挂载路径在 4.0 " +
        "上不成立。这与「依赖私有回调表」是**两条相互独立**的依据。",
    },
    summary:
    ...
    residualRisks: [
      ...
      "即使去掉统计脚本，`_rd` 这一条也不会因为本库的改动而消失：它是脚本自己的传输实现，" +
        "要修只能改上游（M8 / #43）。",
      "`View` 构造失败的根因未继续排查（不知道是缺 pane、还是期望 legacy 容器 API）；" +
        "本库不修它，只把结论写清楚。",
    ],
  },
];

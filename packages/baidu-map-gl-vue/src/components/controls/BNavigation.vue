...
export interface BNavigationProps {
  anchor?: string;
...
/**
 * BNavigation —— 平移缩放控件（官方 `NavigationControl`）
 *
 ...
 */
const props = withDefaults(defineProps<BNavigationProps>(), {
  anchor: "BMAP_ANCHOR_TOP_LEFT",
  offset: () => ({ x: 30, y: 10 }),
  // 两个布尔选项都显式给出官方声明的默认值（`@default true` / `@default false`）：
  // Vue 对布尔 prop 有「缺省即 false」的转换，不显式声明会让「用户显式传 false」与
  // 「用户没传」不可区分（见 `BMapType` 的同一条注释）。
  showZoomInfo: true,
  enableGeolocation: false,
  visible: true,
});

const spec: ControlSpec<BNavigationProps> = {
  kind: "navigation",
...
defineOptions({ name: "BNavigation" });
</script>

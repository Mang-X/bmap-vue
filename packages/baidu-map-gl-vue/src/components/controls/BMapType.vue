...
export interface BMapTypeProps {
  anchor?: string;
...
/**
 * BMapType —— 地图类型切换控件（官方 `MapTypeControl`）
 *
 ...
 * 不依赖 SDK 的隐式配置）。
 */
const props = withDefaults(defineProps<BMapTypeProps>(), {
...
const spec: ControlSpec<BMapTypeProps> = {
  kind: "map-type",
  options: (p) => ({
    anchor: p.anchor,
    offset: p.offset,

...
export interface BPanoramaProps {
...
/**
 * BPanorama —— 全景查看器（官方 `BMap.Panorama`）
 *
 * M7-CONTROL-PANORAMA / issue #41。与 `<BMap>` 的关系是**并列**而不是嵌套依赖：查看器创建在
 * 自己的容器里（`new BMap.Panorama(container)`），既不挂在 Map 上，也不受地图的暂停/重试策略
 * 管辖；它只需要一个 Client，因此放在 `<BMap>` 或 `<BMapProvider>` 子树里都可以。
 *
 * 命令面与 `<BMap>` 的受控写入同源：`point` / `id` / `pov` / `zoom` / `visible` 都是**写命令**
 ...
 */
const props = withDefaults(defineProps<BPanoramaProps>(), {
  visible: true,
});
...
// 全景只需要 Client：`resolveMapContext()` 在 `<BMap>` 子树里给地图 context、在 `<BMapProvider>`
...
// 卸载顺序：**父的 `onUnmounted` 在子树之后**，因此 `<BPanoramaLabel>` 先把自己从查看器上摘掉，
...
<template>
  <!--
    ...
    **必须保留 `<slot />`**：`<BPanoramaLabel>` 这类子组件要挂载才会创建标注——没有出口的话

...
/**
 * BCustomOverlay —— 自定义 DOM 覆盖物（M5-CUSTOM-MENU / issue #33）
 *
 * 组件只做三件事：**声明 spec**（`./customOverlaySpec.ts`）+ **渲染 slot 到 detached 宿主** +
 * 拥有宿主。实例生命周期、字段更新、事件绑定全在 `useCustomOverlay` / `useOverlaySpec` 里。
 *
 * ```vue
 * <BCustomOverlay :position="{ lng: 116.404, lat: 39.915 }" :offset="{ x: 0, y: -12 }" @click="onClick">
...
export type { BCustomOverlayProps };

const props = withDefaults(defineProps<BCustomOverlayProps>(), {
...
/**
 * `$attrs` 落在宿主内部的包装节点上（与 `<BInfoWindow>` 同一条约定）。
 *
 ...
 */
defineOptions({ name: "BCustomOverlay", inheritAttrs: false });

...
/**
 * BTrafficLayer —— 实时路况图层（官方 `BMap.TrafficLayer`，4.0）
 *
 * 官方把它定义为「预配置的 `TileLayer`」，因此构造选项与 `BTileLayer` 一致（`opacity` /
...
const props = withDefaults(defineProps<BTrafficLayerProps>(), {
...
useLayerResource<BTrafficLayerProps>(props, {
  component: "BTrafficLayer",
  toSpec: (p) => ({
    kind: "traffic",
    visible: p.visible,
    opacity: p.opacity,
...
defineOptions({ name: "BTrafficLayer" });
</script>

 ...
 *    `vue-tsc` 钉住，见 `fixtures/v3-consumer/src/index.ts`）；
 ...
 * - `visible=false` 用覆盖物的 `show/hide`（与 `BMarker` 的显隐口径一致：SDK 无该能力时告警）。
 */
...
import type { BMarkerListProps } from "../../types/components";
import type { MapReadyContext } from "../../core/context/types";
import type { MarkerHandle } from "../../driver/types/handles";

const props = withDefaults(defineProps<BMarkerListProps<Item>>(), {
...
const reports = createProblemReporter("BMarkerList", (message) => devWarn(message));

...
/**
 * BPointLayer —— 扩展 API 的批量点（**单个** SDK 资源，M6-POINT-CLUSTER / issue #35）
 *
 ...
 * 其余按取值。口径与 `BPointCollection` 逐条一致（同一份实现，不另写一套）。
 */
...
function ctorOptions(p: Readonly<BPointLayerProps<Item>>): Record<string, unknown> {
  return {
...
const resource = useNativeLayerResource<BPointLayerProps<Item>>(props, {
  component: LABEL,
  kind: LAYER_KIND,
  ctorOptions,

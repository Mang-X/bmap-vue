 ...
 *
 * 生效范围要说准：换成新实现后生效的是**后续的调用**——下一个瓦片 / 下一次请求 / 下一次创建
 * DOM 元素会用新实现；**已经产生的结果不会回溯变化**（已经加载的瓦片、已经建出来的 DOM 元素
 * 保持原样，`createDom` 尤其如此：它只在 `setData` 驱动的解析里被调用）。而内联箭头仍然不会
 * 触发重建。需要让既有结果也换实现时，属于「只在解析数据时求一次」那一类（见下）。
 *

 ...
 *
 * 「容器获得非零尺寸后才创建 Map」这条门禁需要一个**可测**的读数，而读数在不同环境里的
 * 可靠来源不同：
 *
 * | 环境 | 可靠来源 | 说明 |
 * | --- | --- | --- |
 * | 真实浏览器 | `getBoundingClientRect()` | 小数尺寸、含 transform 后的实际盒子 |
 * | 无布局引擎的测试环境（happy-dom / jsdom） | 同上（由测试基线的**最小盒模型**提供，见 `packages/test-utils/browser-shims.ts`） | 两个 DOM 实现都不做布局，`getBoundingClientRect()` 恒为 0，`offsetWidth` / `clientWidth` 亦然 |
 *
 * 因此本模块只做两件事：**按固定优先级取值**（`getBoundingClientRect` → `offsetWidth/Height`
 * → `clientWidth/Height`）与**判定「能不能拿来建图」**（宽高都必须 > 0）。判定与取值分开，
 * 让「零尺寸」这条规则可以脱离 DOM 直接单测。
 *
 ...
 * 读元素的盒子尺寸。
 *
 * 优先级固定的原因：真实浏览器上 `getBoundingClientRect()` 是唯一能反映 transform 与小数
 * 的读数，而 `offsetWidth` / `clientWidth` 是取整的旧读数。**只要 rect 可读就用它**，
 * 即使它是 0——否则「元素真的被折叠成 0 宽」会被降级读数掩盖掉。
 */
export function readElementSize(element: Element | null | undefined): ElementSize | null {
  ...
  const legacy = element as { offsetWidth?: unknown; offsetHeight?: unknown };
  if (typeof legacy.offsetWidth === "number" || typeof legacy.offsetHeight === "number") {
    return { width: toFiniteSize(legacy.offsetWidth), height: toFiniteSize(legacy.offsetHeight) };
  }

/**
 * useBMapRidingRoute —— 骑行路线规划（headless，M7-ROUTES / issue #39）
 *
 ...
 *
 * 刻意**不**把两者合成一个 `useBMapRoute(mode)`：两个服务在 SDK 里就是两个类、两个能力 id，
...
export function useBMapRidingRoute(options: MaybeRefOrGetter<BMapRidingRouteOptions> = {}) {
  ...
  >(ctx, {
    ...
    create: (context: BMapServiceInvokeContext): ServiceHandle<"service:riding-route"> => {
      const state = readState();
      return jsapiV4ServicesOf(context.client).createRidingRoute(
        requireRouteLocation(state.location, context.map, "useBMapRidingRoute"),

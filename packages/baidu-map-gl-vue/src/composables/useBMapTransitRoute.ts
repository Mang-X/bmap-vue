/**
 * useBMapTransitRoute —— 公交路线规划（headless，M7-ROUTES / issue #39）
 *
...
export function useBMapTransitRoute(options: MaybeRefOrGetter<BMapTransitRouteOptions> = {}) {
  ...
  >(ctx, {
    ...
    create: (context: BMapServiceInvokeContext): ServiceHandle<"service:transit-route"> => {
      const state = readState();
      return jsapiV4ServicesOf(context.client).createTransitRoute(
        requireRouteLocation(state.location, context.map, "useBMapTransitRoute"),

/**
 * useBMapDrivingRoute —— 驾车路线规划（headless，M7-ROUTES / issue #39）
 *
 ...
 *
 * 与官方 UI Kit 的分流：`BRoutePlan`（`baidu-map-gl-vue/ui-kit`）是标准面板，它自己发请求、自己画；
 * 本 hooks 是**完全自定义 UI** 那条路。**同一次界面操作只走其中一条**——两条都接上会让一次点击
 * 发出两次检索（见 `docs/zh-CN/hooks/useBMapDrivingRoute.md` 的「与标准面板互斥」一节）。
 */
...
export function useBMapDrivingRoute(options: MaybeRefOrGetter<BMapDrivingRouteOptions> = {}) {
  ...
  >(ctx, {
    ...
    create: (context: BMapServiceInvokeContext): ServiceHandle<"service:driving-route"> => {
      const state = readState();
      return jsapiV4ServicesOf(context.client).createDrivingRoute(
        requireRouteLocation(state.location, context.map, "useBMapDrivingRoute"),
   ...
   * 要按地址出发请先经 `useBMapGeocoder` / `useBMapLocalSearch` 拿到坐标或 POI。

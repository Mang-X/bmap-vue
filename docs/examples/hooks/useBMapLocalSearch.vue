<template>
  <div>
    ...
    <BMap v-bind="$attrs" :zoom="12" :center="center">
      <BMarker v-if="focused" :position="focused" />
    </BMap>
  </div>
</template>
...
const { data, status, sdkStatus, isLoading, search, gotoPage, clear } = useBMapLocalSearch({
  location: "北京市",
  pageCapacity: 10,
});

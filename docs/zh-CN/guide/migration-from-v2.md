...
>
> SDK 世代的差异（v2 的 `BMapGL` → 3.0 的 `Map`）单独成页：
...
| `usePubSub` | 已移除 | 改用 context/whenReady（`useMap()` + `whenReady()`） |
...
```vue
<!-- v2 -->
<Map ak="xxx" :center="center" :zoom="14" @initd="onInit">
  ...
</Map>

<!-- v3(等价,initd→ready；provider 只决定加载器，ak 走全局插件或 ak prop) -->
<Map ak="xxx" :center="center" :zoom="14" @ready="onReady">
  ...
</Map>
```
...
- `resetCenter()` **已移除**（它是「名字说重置中心、实现重置整个视野」的废弃别名），改用
  `resetView()` 恢复初始视角。
- `<Map ref>` 拿到的是定型后的命令面 `MapExpose`：常用 get / set / pan / fit、
...
```vue
<!-- v2 与 v3 用法一致 -->
<Marker :position="{ lng: 116.4, lat: 39.9 }" @click="onClick" />
```
...
> 大量点请勿堆叠独立 Marker,改用 `MarkerCluster` / `MarkerList` / `BPointShapeLayer`。
>
> 历史上那个 `PointLayer`（「每项一个 Marker」却叫批量层）已在 M6 移除；3.0 里的 `PointLayer` 是
...
```vue
<!-- 推荐 -->
<InfoWindow v-model:open="open" :position="pos" title="北京">
  内容
</InfoWindow>
```
...
| 场景 | 组件 | 说明 |
|---|---|---|
| 少量、逐点交互 | `Marker` | 一 V 一组件 |
| 中等规模、需聚合 | `MarkerCluster` | 数据组件 + 内置网格聚合 |
| 中小规模列表 | `MarkerList` | 每个 item 一个 SDK Marker，由一个组件统一 diff 和清理 |
| 千级以上 | `BPointShapeLayer` | 单个批量 SDK 资源（4.0 原生点图层），拾取回传业务项 |

```vue
<MarkerCluster :data="stations" item-key="id"
...
| code | 旧名 | 替代 | 组件 | 说明 |
| --- | --- | --- | --- | --- |
| `BMAP_DEPRECATED_PROP_ALIAS` | `startPoint` + `endPoint` | `bounds` | `GroundOverlay` | 一个 `bounds`(`{ southwest, northeast }`)取代两个角点。正典有值时旧名**完全不参与**(连提示都不发) |
| `BMAP_DEPRECATED_PROP_ALIAS` | `show` | `open` | `InfoWindow` | 打开状态的主模型统一为 `open`(v2 的 `v-model:show` 仍会读取)。与上一行的差别:这里的正典 `open` 有运行期默认值,「父级没传」不可观测,因此**旧名被显式给出时仍然生效**(两个都传时以旧名为准) |
| `BMAP_DEPRECATED_PROP_ALIAS` | `menuItems` | `items` | `ContextMenu` | 菜单项的数据入口统一为 `items`(v3 的 `menuItems` 仍会读取)。与 `bounds` 同一口径:正典 `items` 有值时旧名**完全不参与**(连提示都不发) |
| `BMAP_DEPRECATED_EVENT_ALIAS` | `@drag-end` | `@dragend` | `Marker` | 两个名字都会发(同载荷),提示同实例一次 |
...
3. 替换 `@initd` → `@ready`(可选)。
4. 检查 Marker:大列表迁移到 `MarkerCluster` / `MarkerList`。
5. 检查 InfoWindow:用 `v-model:open` 替代 `modelValue`。
...
**Q: 多个 Map 会互相干扰吗?**
不会。每个 Map 创建独立 MapRuntime,含独立 event bus / overlay registry。

**Q: 批量点该用哪个组件?**
看「落地成几个 SDK 资源」：逐项 Marker 用 `MarkerList`，聚合用 `MarkerCluster`（默认是一个原生聚合
图层），单个批量资源用 `BPointShapeLayer` / `PointIconLayer` / `PointLayer`（三者的差别只是「落在哪个
原生类上」，见[数据组件](../components/data)）。

**Q: `PointLayer` 这个名字以前不是「每项一个 Marker」吗?**
那是 2.x 的老实现，已在 M6 删除；3.0 的 `PointLayer` 是重新引入的**原生扩展 API**组件

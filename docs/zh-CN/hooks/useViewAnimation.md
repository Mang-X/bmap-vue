...
```ts
import { useViewAnimation } from 'bmap-vue'
```
...
```ts
const { start, cancel, status, ready } = useViewAnimation(options, map)
```

:::tip
该 hooks 需要地图 ready（`BMapClient` 就绪）后才能创建动画实例；在 `<Map>` 子树内调用时可省略 `map` 参数
:::
...
`start()` 会先取消上一段再起播新段，但这个清场**按两条路径交付**（2026-09-21 真实 4.0 实测）：

| 上一段的状态 | 清场的交付时机 | 实测形状 |
| --- | --- | --- |
| **已启动**（已进启动安全窗口） | **即时**：取消成功返回后才提交新段 | 两者相隔约 70ms |
| **待启动**（还在启动窗口内） | **延后**：此刻 SDK 取消必抛 `TypeError`，只能登记请求 ⇒ **新段先提交** | 取消落在旧段**自己的** `animationstart` 上，与它相隔 0.0–0.3ms |

...
const { data, status, sdkStatus, isLoading, supported, search, gotoPage, clear, cancel } =
  useBMapLocalSearch(options)
```

:::tip

- 本 hooks 只需要 **Client 上下文**：`<BMap>` 或 `<BMapProvider>` 子树内都可用，**不需要地图实例**；
- **要绘制官方结果**（地图标注 / 结果面板 / 自动视野）必须在 `renderOptions.map` 里显式传
  `MapHandle`（`<BMap>` 的 `@ready` 或 `useBMap().map`），不传就是纯 headless；
...
同一次交互只会走其中一条（如果你同时用（例如）`<BPlaceSearch>` 与本 hooks，那是两条独立请求，
由你决定是否合并）。
...
export declare function useBMapLocalSearch(

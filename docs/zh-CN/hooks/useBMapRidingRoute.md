# useBMapRidingRoute 骑行路线规划

基于百度 `RidingRoute` 的 **headless** 骑行路线规划。公开签名与
[useBMapWalkingRoute](./useBMapWalkingRoute.md) **完全一致**（地名 / 坐标 / POI 起终点，没有途经点，
...
与 `useBMapDrivingRoute` **完全一致**（状态口径、归属、`clear` / `cancel` / `reset` 都一样），

# useBMapWalkingRoute 步行路线规划

基于百度 `WalkingRoute` 的 **headless** 步行路线规划：与
[useBMapDrivingRoute](./useBMapDrivingRoute.md) 共用状态口径、归属模型、绘制所有权与
...
与 `useBMapDrivingRoute` **完全一致**（`data` / `status` / `error` / `sdkStatus` / `isLoading` /

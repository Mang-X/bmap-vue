...
**互斥通知以 SDK 的实际情况为准**：同图互斥的「谁被顶掉了」不只看我们发过哪些命令，也看 SDK 真实的
`open` / `close` 事件 —— 所以**外部**（别处调 SDK）打开或关闭气泡时，被顶掉的一方同样会收到
`update:open false`，不会停在一个「模型说开着、地图上其实已经没有」的状态。
而**过期回包**（例如重开确认先到、上一次关闭的回包后到）不会改变任何一方的判定：那类事件只用来结账，
组件的模型与地图归属都保持原样。**用户点关闭按钮**（`clickclose`）则是**带来源的真实关闭** ——
它一定会生效（回写 `update:open false`），不会被上面那种「旧回包」的判定吞掉。

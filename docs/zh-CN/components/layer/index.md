...
- `removeLayer` 抛错时，**挂载状态是未知的**（SDK 可能已经摘掉、也可能没有）——两种猜测都危险：
  ...
  这个确定性动作收敛；同时**保留**「仍需摘除」的记账，后续的永久销毁（组件卸载 / 重建 / Map 销毁）
  会**再试一次**，不会把实例永远留在图上。如果**最后一次**摘除也失败，则只剩 `logger.warn` 可观测
  （真实收口在 SDK 自己的 `map.destroy()`）。
...
- **释放**：组件卸载、地图销毁（含 `keepAliveBehavior="dispose"` 的停用）都会摘掉图层并释放
  监听；诊断计数归零有测试锁住。数据驱动图层（DOM / GeoJSON）在这条路径上**保证最终清空**
  （`removeAllOverlays()` / `clearData()`）。如果 `visible=false` 已经先成功摘过一次，永久销毁做的
  就只是 **detached cleanup**（清空 + 结束），**不会**再调一次 `removeLayer`——官方没有承诺

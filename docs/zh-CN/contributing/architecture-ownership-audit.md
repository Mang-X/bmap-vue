...
- **A2**：`useBMapViewAnimation` 重写成公开面（`start` / `cancel` / `status` / `ready`），删除私有成员读写与 `setKeyFrames` 两段式。补上的首份行为用例（该 hook 原先零覆盖）另外暴露出两处既有缺陷：取消原先排在微任务里，而 `<BMap>` 在父组件 `onUnmounted` 销毁地图 ⇒ 每次卸载抛一个无人接收的 `BMAP_RESOURCE_DISPOSED`；监听释放原先共用一个槽位 ⇒ 被取代那段的 `animationcancel` 会摘掉新段的订阅并把状态写回 `idle`。两处都改为**每段自带现场 + 同步取消**。评审再追出一处同源的证据问题：旧段的监听只在收到 `animationcancel` 时才释放，而该事件是否派发正是 F-1 未证的那条前提 —— 现在接管时同步释放，**本库自己的记账不再等 SDK 回调**。

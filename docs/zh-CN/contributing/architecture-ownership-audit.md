...
| `isControlled`（`ControllableState` 返回值之一） | `composables/useControllableState.ts` | **生产 0**（`<Map>` 用即时的 `value() !== undefined`） | 零消费者 —— 按 AGENTS.md 本应删；但它挂在**已发布的公共 composable** 的返回类型上，属冻结契约 | **KEEP + 标注** | #137 复核：删除是破坏性变更，与「不改动已冻结的公共语义」的验收冲突。处置＝**保留并在源码 JSDoc 与 `docs/zh-CN/hooks/useControllableState.md` 注明「库内无消费者、供公共契约」**，避免下一个读者去找不存在的调用点。AGENTS.md 的零消费者规则针对**内部**面，冻结公共成员是例外；要移除须另开破坏性变更票（ADR §6.2） |

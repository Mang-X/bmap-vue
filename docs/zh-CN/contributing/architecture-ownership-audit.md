...
| 告警档位 `mode`（`"controlled" \| "uncontrolled"`） | `composables/useControllableState.ts` | **生产 0**（`warningsEnabled` 为假时是 `undefined`） | 唯一消费者是「受控 ↔ 非受控」那条 dev warning，**不参与** `value` / `internal` / 容差相等 / `reset()` / SDK reconcile。却是无条件初始化 + 无条件维护（构造期多读一次 `value()`、每次 `syncExternal()` 改写一次） | **KEEP（按条件存在）** | #137 复审十轮：watcher 注册 / `warnOnce` 短路 / `mode` 本身**统一读 `warningsEnabled = warn && isDev()`**。这正是验收第 2 条要消除的「为 controlled/uncontrolled 术语付不必要 runtime」的字面形态。⚠️ 用例数的是 **`value()` getter 调用次数**（production / `warn:false` 为 1，development 为 2）——它既非 effect、也不改变任何可观察结果，行为断言与 effect 计数都分辨不出。ADR §6.2 |

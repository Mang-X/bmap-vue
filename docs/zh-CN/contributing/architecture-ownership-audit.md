...
| 同 hooks 的 `viewAnimation` 句柄 ref + `setKeyFrames` 两段式 + `disableDragging` | 同上 | 只有文档 | 句柄每次 `start()` 都被换掉、`disableDragging` 从未生效 ⇒ 退化成常量 | **REMOVE** | #104 A2 一并删除；`status` 改成只由公开事件写的**观察值**（`idle` / `playing`） |
...
| 同一处前提在 hooks 侧的**残留用法**：`useBMapViewAnimation` 里被取代那一段的三条监听，原先只由该段的 `animationcancel` 释放 | `composables/useBMapViewAnimation.ts`（`AnimationRun` / `start`） | 该 hook 自己（`EventDriver.groups` 是强引用 Map，未释放即泄漏） | 与上面同一条 `FAKE-ONLY` 时序：Fake 里「Driver 起播前取消上一段 ⇒ 上一段必然派发 `animationcancel`」成立，真实 SDK 未证 ⇒ 本库自有记账的销账被挂在一个**不由我们决定的事件**上 | **SIMPLIFY（本票已做）** | #104 A2 收尾 + #105 评审 P1：**归属与释放分开**。① 一段动画的**归属**（`current`）只在取消被 SDK 接受之后交回——`MapDriver` 的契约是「取消失败时记录保留，可再次 stop/destroy 重试」，提前清空就等于 hook 自己先丢掉重试入口（`cancel()` 第二次直接 no-op，SDK 那边却还在播）；② 一段动画的**订阅**是本库自有记账，不等 SDK 回调：起播被拒的那一段（两阶段提交，`start()` 随之 reject、旧段继续被观察）、半途绑定失败的那一段、以及卸载时的当前段，都当场释放。`status` 仍只由公开事件写。文档随之删掉「由 Driver 在起播前同步取消上一段」的时序承诺，并把「取消是地图级命令、不保证不牵连同图其它动画」写进 `cancel` 的口径。四条回归见 `v3-useBMapViewAnimation.test.ts` 的「取消失败时保留重试入口」 |
...
| `runtime` 能力族四条（`resource-scope` / `capability-override` / `fake-sdk` / `async-task`） | 同文件（原 `:745-776`） | **没有任何** `supports()` / 组件 / 测试按 id 问过；只被「每个 family ≥1 条」这条断言养着 | 类别错误（拿自家模块冒充实测风险等级） | **REMOVE** | #104 R10：整族连同 `CapabilityFamily` / `CAPABILITY_FAMILIES` 一起删，矩阵与 JSON 重生成（62 条能力），「每个 family ≥1 条」的循环改成五族 |
...
| `DataLayerManager.sync(items, getKey, itemVersion, force)` 的 `itemVersion` 形参 | `core/data/DataLayerManager.ts:34` | 两个 data 组件都在传，但 `apply()` **从不读**它（R6 删掉 `shouldFullReplace` 之后彻底没有实现） | 无消费者的参数＝假支持；但类挂在 `./core` 公共出口上，外部调用方按位置传参时会把第 4 个实参错位成 `force` | **SIMPLIFY** | 随 **#44** 的 `./core` 出口收窄一起删（与 `SdkResourceAdapter` 同批）。组件的 `dataVersion` prop **保留**：它的真实作用是 `watch([data, dataVersion])` 触发重新 diff（数组引用不变、item 换对象时靠它更新位置），与被删的那个形参无关 |
...
| `driver-contract` 的 `expectation: "fixture" \| "live"` 两档 | `test-utils/driver-contract.ts:649` | 两个调用方都传 `"fixture"`，`"live"` 零使用者 | **SIMPLIFY** | 后续票：删档，或真的接到 `smoke-jsapi-v4.mts --mode=live` |
...
| --- | --- | --- | --- |
| F-1 | `startViewAnimation` 的启动窗口：`animationstart` 是否真的在内部 Animation 构造前同步派发、启动前 `cancelViewAnimation` 是否真的抛 `TypeError` | 只有 `FakeV4ViewAnimation` 建模 | Driver 的 teardown 按防御模型保留；对外只承诺「`stopViewAnimation` 之后以 `animationend` / `animationcancel` 为准」 |
...
- **A2**：`useBMapViewAnimation` 重写成公开面（`start` / `cancel` / `status` / `ready`），删除私有成员读写与 `setKeyFrames` 两段式。补上的首份行为用例（该 hook 原先零覆盖）另外暴露出两处既有缺陷：取消原先排在微任务里，而 `<BMap>` 在父组件 `onUnmounted` 销毁地图 ⇒ 每次卸载抛一个无人接收的 `BMAP_RESOURCE_DISPOSED`；监听释放原先共用一个槽位 ⇒ 被取代那段的 `animationcancel` 会摘掉新段的订阅并把状态写回 `idle`。两处都改为**每段自带现场 + 同步取消**。#105 评审第二轮又追出三处同源的失败路径问题（详见第 2 节那一行）：归属被提前清掉（丢重试入口）、新段在起播被拒前就提交现场、卸载中取消失败打断钩子并留下订阅；已按「归属与释放分开」重构，并补 4 条回归。
...
- **R1–R11**：全部落地，逐项删净实现、出口、引用、生成物与文档承诺（矩阵与 JSON 重生成后为 **62 条能力**；`v3-default-loader-boundary` 从「调用它」改成「断言它不存在」）。R6 删掉 `shouldFullReplace` 之后暴露出的 `DataLayerManager.sync` 死形参已登记在第 5 节，随 #44 收口。
...
**尚未交接的一项（不是遗漏，是授权边界）**：#104 的验收里有两条要写到 GitHub 的项——把本表链接进 #12、
以及把 Ownership-first / Evidence-before-abstraction 四条写进 #12 与所有未开工的 roadmap issue。
本轮按「本地文档 + 提交，暂不改 GitHub」执行：约束已经进 `AGENTS.md`（对所有会话生效），本表是事实源，
但 **#44 / #43 的 issue 正文里目前没有这些行**。GitHub 侧落笔前，请以本表第 5、6、7 节标
**SIMPLIFY / 后续票** 的行作为待办清单，不要假定对面已经知道。

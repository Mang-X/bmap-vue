...
**#140**：与 `@baidumap/vue-bmap@1.0.1`（票面**精确锁定**的基线版本）在 10 个场景上的**同场景**
对照。它与上面的「单库趋势基线」不是一回事：那条与自己的上一次提交比，这条与**官方同版本**比，
门禁口径也不同。
...
**为什么它能在 CI 里当门禁**：无 AK、无网络、完全确定。官方库经 `@baidumap/jsapi-loader` 的
`load()` 检测到已存在的 `window.BMap` 就**直接复用**，所以两侧都跑在本仓库的 Fake v4 上
（机制见 `tests/performance/officialContrastHarness.ts` 文件头）。两侧在**同一个 vitest 进程**
里跑，共享同一份 Vue 与同一份确定性数据——跨进程各跑一次满足不了「同环境」。

### 怎么读

报告**分三节**，这是「不伪造官方等价物」在输出层的落点：

1. **可比场景**：两边都有官方等价物的才进这张表，每行带差值。差值**只作读数**。
2. **本库扩展档**：官方无等价契约的场景**单列**，**永不**与任何官方数字并排。理由逐条写出来
   （PointCollection 在 4.0 已移除 / 官方 binding 无原生点图层封装 / 官方库无 KeepAlive 语义）。
3. **不变式**：架构预期，与快慢无关。
...
被测对象：本库**源码**（`packages/bmap-vue/src/**`）——**不是**打包产物；引擎：**Fake v4 替身**（`fake-v4`）——**不是**真实 JSAPI，本轮无 AK、无网络，官方库经 `jsapi-loader` 复用已存在的 `window.BMap` 跑通。

> 毫秒为**该引擎 / 上述机器的同轮读数**，仅用于解释此次实验，**不是跨机器阈值**，也**不代表**真实 JSAPI + 浏览器下的耗时。本节不按快慢排序，也不给百分比或倍数——票面禁止营销式排名。
...
- `map-cold-mount`：`listen` 调用面 本库 43 / 官方 5（本库多 38）；组件渲染本库 5 / 官方 5（持平）。
- `marker-100-mount`：`listen` 调用面 本库 143 / 官方 1105（本库少 962）；组件渲染本库 6 / 官方 105（本库少 99）。

**本库更贵的地方在这里**：`map-cold-mount` —— 组件与生命周期抽象在这些路径上要多付 listen 绑定或渲染。代价换来的东西在下一节（卸载残留归零、大数据更新不重建实例）；哪一边的**毫秒**更小由上表说话，本文不替他下结论。
...
收益在这里是**可复现的架构差**，不是快慢：组件渲染次数：`marker-1k-update` 本库 2 / 官方 1001——高频更新下本库不必重渲整棵树；卸载后残留：`marker-1k-update` 本库 0 / 官方 1000——覆盖物确实被摘掉了。这些是 #138 Vue-native 收口真正要防回归的东西，也是 CI 里不变式门禁盯的读数。（另有 4 个场景的 `recreate` 与 SDK 调用面**两侧持平**——持平的不是收益，只是没有回退。）
...
- **官方对照档（Fake 档）同样如此**：它比的是「同一替身下两库建了多少资源、发了多少 SDK 写入、
  卸载后剩多少」，**不是**浏览器里的真实表现。真实浏览器档见「官方对照档」一节——本轮**无 AK、
  未实跑**。

...
同步耗时最长 24ms（安静档 3 ~ 24ms）**，明显低于浏览器 50ms 长任务线，因此本库**不引入 Worker**。
...
- **归一化列**：`min / calibration.cpu`。校准工作量是一次固定的纯计算，用来抵消机器速度差；
  跨机器只比这一列，绝对毫秒只作参考。
- **比值列**：与提交基线 `tests/performance/baseline.json` 的归一化比值。默认阈值 5×：基线录在一台
  机器上、门禁在另一台（CI 共享 runner）上跑，5× 是为了让跨机器噪声不可能染红门禁，同时仍拦得住
  「一个数量级」的回退。

...
### 23. 成本是**某条路径**上的量，不是可加的标量

八轮评审抓到本仓最实的一处：简单路径那节把 `listen` / 渲染**求和**后只报一个方向
（「本库少 924」），而它**上面那张表**里 `map-cold-mount` 是 43 / 5（**本库多 38**），
只有 `marker-100-mount` 才是 143 / 1105。两边方向相反，求和既**与相邻表格直接矛盾**，
又把「本库在哪条路上更贵」**抹掉**了——而「暴露简单路径成本」正是票面点名要的东西。

ADR 21 曾把判据钉成「总量比较」（`oursTotal > offTotal`）：门禁固化的正是一个会产生错误
结论的口径。成本发生在**某条路径**上，求和把「在哪儿贵」这个唯一的结论变量压成了一个
没有意义的合计。改法：**逐场景**列方向，并**点名本库更贵的那几条**。门禁因此改成断言
「方向相反的两行都如实出现」+「本库更贵的路径被点名」，且读数反转时措辞必须跟着消失。

这条与「不得预设本库更贵」不冲突：现在报告**主动**说出本库在 `map-cold-mount` 上更贵，
那正是票面要的东西；它禁的是**写死**结论，不是**没有**结论。

### 24. 两个判别器必须一起判；类型校验逐条钉

- `mode` 与 `engine.kind` 各自放行时，`mode: "fake-v4"` 配 `kind: "real"` 能过全部校验，
  而渲染层会照 `kind` 印出「引擎：真实 JSAPI 4.0」——决策 22 要堵的冒充，从上一层又通了。
  两者必须指向同一档（`REFERENCE_MODE_KIND_MISMATCH`）；
- 决策 22 引入的引擎校验**没有单测碰过**：招牌修复是未验证的。现逐条注入，
  `ENGINE_MISSING` / `KIND_UNKNOWN` / `VERSION_MISSING` / `PAIR_MISMATCH` /
  `OURS_UNDER_TEST_UNKNOWN` / `MODE_KIND_MISMATCH` 六条**各自**验证能红；
- `sourceCommit` 核对在**浅克隆**里取不到那个 commit——那是环境没拉全，不是快照说谎。
  两者必须分开：贡献者在自己机器上不该看到「快照在说谎」的假警报，而真造假也不该被同一条
  噪音盖住。CI 两个相关 job 都用 `fetch-depth: 0`，判据照常生效。

### 25. 成本只比**同一个调用面**；「更贵」的判据要与它列出的项**同域**

九轮评审抓到 `sdkCalls` 的单位问题：`callKind` 决定这个数字数的是哪个面（`reference.mts`：
「**不是**总数」）。决策 23 重写成本段时**丢了**那条守卫，于是 43 次 `setPosition` 可以和
5 次 `listen` 并排，被说成「本库多 38」——那是拿两个不同的东西做减法。两侧 `callKind`
不一致时**不列**，并如实说明「无可比的调用面读数」。

另一处自相矛盾：`heavier` 只看 `sdkCalls`，而那一行**同时**报 `renderCallbacks`，于是
「调用面便宜、渲染更贵」的场景被判成「本库不更贵」，与它自己上一行打架。判据因此与
**列出的两项同域**：任一项更贵就算更贵。

### 26. 高级路径的「收益」只许说**两侧真的不同**的列

上一版写死「更新走 `setPosition` 复用实例而不重建覆盖物、父级无关更新不重发 `setPath`」
——而表里 `recreate` 是 **0/0**、`setPosition` **1000/1000**、`setPath` **0/0** 与 **1/1**：
**全是平的**。把持平的列说成「收益」，而真正分出高下的 `render`（2 / 1001）与
`retained`（0 / 1000）**一个都没点名**：那不是解释，是指错方向。

收益段因此改为**从读数里挑**（哪个列差得最大点哪个，并指名场景），并把持平的列**明说**
为「持平的不是收益，只是没有回退」。若某轮读数不再显示优势，文案会照实说
「本轮高级档没有本库占优的结构读数；按票面『结果使用规则』应回到 #124/#138 复看」。

### 27. 渲染层**不信任**入参的 `engine`；`scripts/**` 不在任何类型门禁里

- 上一版只断言**校验器**发码，而危害发生在**渲染层**：把渲染器改成永远打印 Fake 措辞，
  全部门禁**照样全绿**。渲染时 `mode` 与 `engine.kind` 因此**一起看**，不一致就按本档
  真实跑法书写并注明快照自相矛盾——渲染器不该成一台「把 Fake 读数说成真实 JSAPI」的机器；
- ⚠️ 连带纠正一条**我写的假话**：注释里「真出第二种 mode 时 TS 会在这里报错」**不成立**，
  实测两条都不对——① `toReferenceEnvironment` 根本不收 `mode` 参数，硬编码的 `dom` 与
  mode 没有类型关联；② `scripts/**` **不在任何 tsconfig 的 include 里**
  （`tsconfig.tests.json` 只管 tests、`tsconfig.json` 只管 packages），本文件靠
  `--experimental-strip-types` 跑，塞一个类型错误照样 exit 0。注释改为如实写「要人来改」。

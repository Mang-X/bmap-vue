...
| 页面可见性联动（hidden/offscreen 自动暂停） | 自动 pause/resume 若会改变用户可观察状态，必须是**显式 opt-in** 而不是基础默认；官方语义未验证 | **#110 已收口**：live 探针实测「SDK 不会自行暂停」⇒ 默认只停本库观察；`pauseOnHidden` prop 是 opt-in，且用户自己 pause 过的不被 visibility 抢走 |

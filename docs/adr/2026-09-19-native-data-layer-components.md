...
| 页面可见性联动（hidden/offscreen 自动暂停） | 自动 pause/resume 若会改变用户可观察状态，必须是**显式 opt-in** 而不是基础默认；官方语义未验证 | **#110 已收口**：live 探针实测「SDK 不会自行暂停」⇒ 默认只停本库观察；`pauseOnHidden` prop 是 opt-in，且只对**命令意图仍在播**的实例 pause/resume（`stop()`/idle/用户 pause 不被反向启动，#132 P2）；visibility 暂停记账**绑到被 pause 的 handle**——hidden 期间换实例后 shown 只清账、不对新实例补 resume（#132 评审 P2） |

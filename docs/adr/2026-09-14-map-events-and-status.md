...
## 评审修正（2026-09-14 第四轮，注释一致性）

第四轮复审结论是「上一轮两条已正确处理，未再发现阻塞合并的问题」，只留 2 条**非阻塞的注释一致性**
小项。两条都成立，且**零行为变更**（只动注释）：

| 评审意见 | 事实核对 | 处置 |
| --- | --- | --- |
| **[非阻塞 1]** `useMapEvent.ts` 文件头仍写着 `destroy`「登记在上下文 ResourceScope，因此组件卸载不会摘掉它」，与决策 12 后的语义漂移 | **成立** | 文件头重写为「按『谁在消失』分档」，并点明判据 `MapContext.isTearingDown()` 与它的置位时机 |
| **[非阻塞 2]** `MapRuntime.ts` 的 `mapCreatedCallbacks` 字段注释仍写「建图时清空」 | **成立** | 改为「不在建图后清空，每个注册存活到它自己的 disposer 或 `dispose()`」，附上 `retry()` 那条理由 |

**自查补的同类漂移 2 处**（评审未点名）——按「改注释一致性时把同类行一次扫完」的习惯，全仓搜
`组件卸载不会摘` / `建图时清空` / `不随组件卸载释放` 三个说法，又抓到还在描述旧语义的两处：

| 位置 | 原文问题 | 处置 |
| --- | --- | --- |
| `eventCatalog.ts` 的 `MAP_CONTEXT_OWNED_EVENTS` 说明 | 「订阅登记在上下文的 `ResourceScope` 上，随地图一起释放」容易被读成**一律不摘** | 补「**登记 ≠ 一律不摘**」，指向决策 12 |
| `useMapEvent.ts` 内部 `contextOwned` 字段注释 | 「不随组件卸载释放」同样只剩一半 | 改成「只有整图 teardown 时才留给上下文收尾，子组件自己卸载照常释放」 |

ADR 第一/二轮修正节里保留的「建图后清空注册」是**发现记录**（描述当时被修的缺陷），不是现状描述，
刻意不改。

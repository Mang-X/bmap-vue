...
5. **`OverlaySpec` 只覆盖「加到地图上的覆盖物」这一形态**：挂载/摘除固定走
   ...
   而不是现在造一个没有消费者的参数。
   > 补充（[ADR 2026-09-18](./2026-09-18-infowindow-host-and-ownership.md)）：`<BInfoWindow>` 是**第二类**
   > 同样用不了本层的组件 —— 它不是「加进地图的覆盖物」（打开 / 关闭是 Map 级 API，
   > `add` / `remove` 对它必须抛错），因此有自己的一套「打开意图收敛 + 每地图账本」。两条限制指向
   > 同一个 `add` / `remove` 级扩展点。

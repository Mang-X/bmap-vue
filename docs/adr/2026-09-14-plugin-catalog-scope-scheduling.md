...
代价是「这个名字再也没人 acquire」时候选会多留一会儿（有界的保留），换掉的是「把在用的资源拆掉」这个更坏的失败。同理，**`setup` 必须在「这次加载还算不算数」校验之后才执行**（评审第三轮 P1-2）：跑在校验之前的话，地图销毁后还会冒出一个 `setup`，而且 `setup` 抛错会把 promise 推进 catch、绕过上面那条过期结算的释放路径 —— 实例就此泄漏。

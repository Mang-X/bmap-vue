...
| [2026-09-21](./2026-09-21-plugin-load-channel-timeout.md) | 插件脚本加载通道的超时与取消语义（保持两条通道分离的判据 / **内置工厂** 60s 默认超时与「作废」清理、公共工厂语义不变 / 取消语义三条读数 / 挂起隔离的边界；取代 `2026-09-13-plugin-compat-inventory` 的「插件加载没有超时」与 `2026-09-21-plugin-verdicts-and-advanced-freeze` 已知限制 8 的后半句） | Accepted |

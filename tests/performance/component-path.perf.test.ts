...
describe("§5 输入数据的响应式形态决定更新成本（阶段 A 结论的关键对照）", () => {
   ...
   *
   * #124 的**深响应子问题**据这一条落地：3.0 的落地方式是**文档指引**（大数据量用 `shallowRef` /
   * `markRaw` + `dataVersion`），实现级 `pauseTracking` 取证有效（组件路径 ~2.7×）但因公开边界 /
   * 模块身份耦合不落地。取证读数与理由见 ADR `2026-09-24-deep-reactive-array-update-path`；
   * #124 其余范围（scheduler/batching 取证）留在 #124。
   *

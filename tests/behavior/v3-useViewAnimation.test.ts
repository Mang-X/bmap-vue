/**
 * useViewAnimation（#104 A2 之后的行为面）
 *
...
import { useViewAnimation } from "../../packages/bmap-vue/src/composables/useViewAnimation";
import type { ViewAnimationKeyFrames } from "../../packages/bmap-vue/src/composables/useViewAnimation";
...
type Hook = ReturnType<typeof useViewAnimation>;

function mountHook(run: (hook: Hook) => void | Promise<void>) {
  const Child = defineComponent({
    setup() {
      const hook = useViewAnimation({ duration: 10_000, delay: 0, loop: "INFINITE" });
...
describe("useViewAnimation：只用官方公开面", () => {
  ...
  it("卸载停掉在飞的动画、释放订阅，Fake 侧无资源残留", async () => {
    ...
    fake.diagnostics.assertNoLeaks("useViewAnimation 卸载");
  });
});
 ...
 *   不能因为等一条可能不来的事件就一直挂着。
 */
describe("useViewAnimation：取消失败时保留重试入口", () => {
  ...
  it("卸载时取消失败：不打断卸载、本段订阅归零，并把失败经 resource:error 交出来", async () => {
    ...
    expect(errors[0]!.component).toBe("useViewAnimation");
    expect(errors[0]!.error).toMatchObject({
      message: expect.stringContaining("cancelViewAnimation"),
    });
    fake.diagnostics.assertNoLeaks("useViewAnimation 卸载时取消失败");
  });
 ...
 */
describe("useViewAnimation：取消被接受之后不再越权", () => {
  ...
  function mountTwoHooks() {
    ...
    const makeChild = (index: number) =>
      defineComponent({
        setup() {
          hooks[index] = useViewAnimation({ duration: 10_000, delay: 0, loop: "INFINITE" });
 ...
 */
describe("useViewAnimation：deferred 与已交付的取消走不同的收尾", () => {
 ...
 */
describe("useViewAnimation：收尾按动画身份收敛", () => {
 ...
 */
describe("useViewAnimation：未交付的旧段仍归本 hooks 所有", () => {
 ...
 */
describe("useViewAnimation：idle 不是串行 barrier（#122 复审 P1）", () => {

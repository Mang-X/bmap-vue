/**
 * PanoramaControl / BezierCurve / MapMask / Marker3D 行为门禁
 *
...
describe('Map extra overlays/controls', () => {
  ...
  it('MapMask 在 v4 上是显式失败（Fake v4 的命名空间没有 MapMask）', async () => {
    ...
    await flushPromises()
    // 原来是：`fake.diagnostics.overlaysCreated` 为 1 且 mask 的 points/options 落在覆盖物上。

/**
 * CustomOverlay：detached 宿主、slot 所有权与事件（M5-CUSTOM-MENU / issue #33）
 *
 * 字段级的 create / mutable / recreate / visible / 事件面**不在这里**：那五条由
 * `v3-overlay-suite.test.ts` 的表驱动用例统一覆盖（`CustomOverlay` 已登记进 `CASES` /
    ...
    defineComponent({
      setup: () => () =>
        h(Map, { provider: harness.provider() }, () =>
          h(CustomOverlay as never, state.value as never, slot ? { default: slot } : undefined),
        ),
    }),
...
describe("CustomOverlay 的宿主与 slot", () => {
  ...
  it("slot 渲染进 detached 宿主，宿主由 SDK 搬走（不落在地图容器顶层）", async () => {
    ...
    harness.assertIdle("CustomOverlay 卸载");
  });
  ...
  it("posiiton 变化走 setPoint(point, true)：业务 DOM 工厂**不被重新调用**", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("CustomOverlay setPoint");
  });
  ...
  it("构造期属性（anchor）变化：重建实例但**复用同一个宿主**——slot 子树不被重建", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("CustomOverlay 重建");
  });
  ...
  it("事件来自事件矩阵：SDK 派发 click / mouseover / mouseout 时组件原样转发", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("CustomOverlay 事件");
  });
  ...
  it("卸载后宿主不残留在文档里（风险章的 Teleport host 残留）", async () => {
    ...
    harness.assertIdle("CustomOverlay 卸载残留");
  });
  ...
  it("SSR 渲染不炸（宿主在服务端不存在，slot 不渲染）", async () => {
    ...
    const app = defineComponent({
      setup: () => () =>
        h(Map, { provider: harness.provider() }, () =>
          h(CustomOverlay as never, { position: { lng: 116.4, lat: 39.9 } }, () => h("div", "x")),
        ),
    });

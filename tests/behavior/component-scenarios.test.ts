 ...
 *
 * 原文（一份已随 webgl-v1 删除的双引擎组件矩阵）回答的是「同一份组件代码在 webgl-v1 与 jsapi-v4
...
describe("Map 视野的受控 / 非受控（M4-STATE / #27）", () => {
  ...
  it("缺省档用库默认视野，且用户交互同样回写（不依赖受控 prop）", async () => {
    ...
    // 与文档的 props 默认值一致（docs/zh-CN/components/map.md 的库默认视野）
...
describe("MapHandle / 容器门禁 / 可见性策略（M4-HANDLE-UX / #29）", () => {
  ...
  it("并发 retry 共享同一次 boot：ready 只广播一次（复审 P2）", async () => {
    harness.failNextInitializeView();
    const { wrapper, bmap } = await mountControlledMap(controlledViewProps);
    const api = exposeOf(bmap);
    expect(statusOf(bmap)).toBe("error");
...
describe("图层套件在同图共存时的领域行为（M7-LAYERS / #40）", () => {
   ...
   * 「挂载数」，卸载后过一遍泄漏门禁。详细的分 kind 断言在 `layer-suite.test.ts`
 ...
 * `native-data-layers.test.ts`，那份文件按 issue 的验收条目组织）。
 *

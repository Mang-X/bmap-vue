/**
 * M4: Marker 动态更新与幂等可见性（Fake v4 后端）
 *
 * 验证关键修复:
...
describe('Marker field-level updates', () => {
        ...
        setup() {
          return () =>
            h(Map, { provider: harness.provider() }, () => [
              h(Marker, { position: position.value }),
            ])
        },
      }),
      { attachTo: el },
  ...
  it('toggles visible without duplicate addOverlay calls', async () => {
    ...
    const wrapper = mount(
      defineComponent({
        ...
        setup() {
          return () =>
            h(Map, { provider: harness.provider() }, () => [
              h(Marker, { position: { lng: 116.4, lat: 39.9 }, visible: visible.value }),
            ])
        },
      }),
      { attachTo: el },
    ...
    await unmountAndSettle(wrapper)
    harness.assertIdle('Marker visible 切换后卸载')
  })
    ...
    const wrapper = mount(
      defineComponent({
        ...
        setup() {
          return () =>
            h(Map, { provider: harness.provider() }, () => [
              h(Marker, { position: { lng: 116.4, lat: 39.9 }, title: 't' }),
            ])
        },
      }),
      { attachTo: el },

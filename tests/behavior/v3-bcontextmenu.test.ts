/**
 * BContextMenu / BMenuItem / BMenuSeparator（M5-CUSTOM-MENU / issue #33）
 *
 * 本文件同时是**行为契约**与**声明面门禁**：issue 的「测试要求」逐条落在这里
 * （数据/声明式一致、target 切换、动态 items/disabled/separator/select payload、旧 alias 告警去重、
 * SSR），外加两组交叉核对（props ↔ 声明表 ↔ Driver 描述符、emits ↔ 事件矩阵）。
 *
 * ## 与 #33 之前的口径差异（**行为变更**，不是修 bug 之外的副作用）
 *
 * `v3-bcontextmenu.test.ts` 上一版把「v4 拒绝把菜单挂到父 Marker」写成了期望行为，理由是
...
/** 收集 `<BMap>` 上下文里的 `resource:error`（组件侧的显式失败通道）。 */
...
/** 含指定弃用 code 的告警（`devWarn` 把 code 放在第二个参数对象里，因此要 `JSON.stringify`）。 */
function warningsWithCode(warn: { mock: { calls: unknown[][] } }, code: string): unknown[][] {
...
async function mountTree(children: () => unknown, host: HTMLElement) {
  const wrapper = mount(
    defineComponent({
      components: { BMap },
      setup: () => () => h(BMap, { provider: provider() }, { default: () => [h(ErrorProbe), children()] }),
    }),
    { attachTo: host },
  );
  await flushPromises();
...
describe("BContextMenu", () => {
  ...
  it("数据 API：菜单挂到地图上，分隔线与 disabled 都落地", async () => {
    ...
    const wrapper = await mountTree(
      () => h(BContextMenu, { items, width: 160 }),
      el,
    );
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 数据 API");
  });
  ...
  it("声明式 API 与数据 API 产出**完全相同**的菜单", async () => {
    ...
    const wrapper = await mountTree(
      () => [
        h(BContextMenu, { items, width: 160 }),
        h(BContextMenu, { width: 160 }, () => [
          h(BMenuItem, { text: "放大" }),
          h(BMenuSeparator),
          h(BMenuItem, { text: "删除", disabled: true }),
        ]),
      ],
      el,
    );
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 两写法");
  });
  ...
  it("声明式：动态显隐的项回到模板里的位置（按渲染顺序，不是注册顺序）", async () => {
    ...
    const wrapper = await mountTree(
      () =>
        h(BContextMenu, null, () => [
          h(BMenuItem, { text: "a" }),
          show.value ? h(BMenuItem, { text: "b" }) : null,
          h(BMenuItem, { text: "c" }),
        ]),
      el,
    );
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 动态项");
  });

  /* ------------------------------------------------------------- target */

  it("写在 <BMarker> 里：菜单挂到该标注上（而不是地图上）", async () => {
    const el = harness.container();
    const wrapper = await mountTree(
      () =>
        h(BMarker, { position: { lng: 116.4, lat: 39.9 } }, () =>
          h(BContextMenu, { items: [{ text: "标记此处", callback: () => {} }] }),
        ),
      el,
    );
    ...
    harness.assertIdle("BContextMenu marker 目标");
  });
  ...
  it("target 切换：标注卸载再挂载后，菜单只挂在新实例上、任何时刻都不重复", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu target 切换");
  });
  ...
  it("visible=false 时不挂载；恢复后挂上", async () => {
    ...
    const wrapper = await mountTree(
      () => h(BContextMenu, { items: [{ text: "x", callback: () => {} }], visible: visible.value }),
      el,
    );
    expect(harness.attached("context-menu")).toBe(0);
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu visible");
  });
  ...
  it("[复审 1] 临时摘挂不屏蔽事件：visible true→false→true 之后 open/close 仍能转发", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 临时摘挂");
  });
  ...
  it("[复审 1] target 迁移后同一菜单实例的事件仍能转发（marker 重建 ⇒ 换目标）", async () => {
    ...
    const wrapper = await mountTree(
      () =>
        h(BMarker, { position: { lng: 116.4, lat: 39.9 }, enableClicking: enableClicking.value }, () =>
          h(BContextMenu, { items: [{ text: "x", callback: () => {} }], onOpen: () => seen.push("open") }),
        ),
      el,
    );
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu target 迁移事件");
  });
  ...
  it("[复审 1] 实例被 replace 释放时，摘除窗口里的 SDK 事件不回放", async () => {
    ...
    const wrapper = await mountTree(
      () =>
        h(BContextMenu, { items: items.value, onClose: () => seen.push("close"), onOpen: () => seen.push("open") }),
      el,
    );
  ...
  it("[复审 2] 只换 callback：不重建菜单，但必须调用新的那个", async () => {
    ...
    const wrapper = await mountTree(() => h(BContextMenu, { items: items.value }), el);
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu callback-only");
  });
  ...
  it("[复审 3] id 一路下发到 SDK：数据 API 与声明式都落地", async () => {
    ...
    const wrapper = await mountTree(
      () => [
        h(BContextMenu, { items: [{ text: "数据项", callback: () => {}, id: "item-from-data" }] }),
        h(BContextMenu, null, () => [h(BMenuItem, { text: "声明项", id: "item-from-children" })]),
      ],
      el,
    );
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu id");
  });

  it("[复审 4] width 默认 100（与 v3 一致），不传时菜单项也拿到 100", async () => {
    const el = harness.container();
    const wrapper = await mountTree(
      () => h(BContextMenu, { items: [{ text: "x", callback: () => {} }] }),
      el,
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu width 默认值");
  });
  ...
  it("open / close 是 SDK 事件：原样转发，不产生任何受控状态", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu open/close");
  });
  ...
  it("select 载荷：两种写法给出同形载荷（item / index / point / pixel / map / target）", async () => {
    ...
    const wrapper = await mountTree(
      () => [
        ...
        h(BContextMenu, { onSelect: (payload: ContextMenuSelectPayload) => fromChildren.push(payload) }, () => [
          h(BMenuItem, {
            text: "定位",
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu select");
  });

  /* ------------------------------------------------------- 集中弃用层 */

  it("旧名 menuItems：可用、告警一次（同实例去重）", async () => {
    const el = harness.container();
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const menuItems = ref([{ text: "a", callback: () => {} }]);
    const wrapper = await mountTree(() => h(BContextMenu, { menuItems: menuItems.value }), el);

    expect(itemTexts(lastMenu())).toEqual(["a"]);
    expect(warningsWithCode(warn, DEPRECATED_PROP_ALIAS_CODE)).toHaveLength(1);

    // 同一个实例里再改一次：仍然是「只警告一次」
    menuItems.value = [{ text: "b", callback: () => {} }];
    await nextTick();
    await flushPromises();
    await nextTick();
    expect(itemTexts(lastMenu())).toEqual(["b"]);
    expect(warningsWithCode(warn, DEPRECATED_PROP_ALIAS_CODE)).toHaveLength(1);

    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 旧别名");
  });

  it("旧名 menuItems：正典 items 有值时完全不参与（连告警都不发）", async () => {
    const el = harness.container();
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    const wrapper = await mountTree(
      () =>
        h(BContextMenu, {
          items: [{ text: "canonical", callback: () => {} }],
          menuItems: [{ text: "legacy", callback: () => {} }],
        }),
      el,
    );

    expect(itemTexts(lastMenu())).toEqual(["canonical"]);
    expect(warningsWithCode(warn, DEPRECATED_PROP_ALIAS_CODE)).toHaveLength(0);

    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 正典优先");
  });
  ...
  it("items 变化时重建菜单，旧实例的监听与资源都归还（不累积）", async () => {
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu 重建");
  });
  ...
  it("width 变化重建菜单，新的菜单项带上新宽度", async () => {
    ...
    const width = ref(120);
    const wrapper = await mountTree(
      () => h(BContextMenu, { items: [{ text: "a", callback: () => {} }], width: width.value }),
      el,
    );
    const menusBase = createdMenus().length;
    expect((lastMenu().items[0] as { options: { width?: number } }).options).toMatchObject({
    ...
    wrapper.unmount();
    await nextTick();
    harness.assertIdle("BContextMenu width");
  });
    ...
    const app = defineComponent({
      setup: () => () =>
        h(BMap, { provider: provider() }, () => [
          h(BContextMenu, null, () => [h(BMenuItem, { text: "a" }), h(BMenuSeparator)]),
        ]),
    });
  ...
  it("声明面：CONTEXT_MENU_FIELDS 覆盖 BContextMenuProps（从类型文件解析，不写字面量）", () => {
    // 键集**从 `types/components.ts` 解析**：写死字面量就成了「字面量自比」，
    // 新增 prop 不会红，这条门禁等于没有（自审 Standards 轴抓到的硬违规）。
    const propsKeys = readPropsKeys("BContextMenuProps").sort();
    expect(propsKeys.length, "BContextMenuProps 的成员解析结果为空").toBeGreaterThan(2);
    ...
    // 旧名必须在集中弃用层登记过（否则它就是「收下但没人读」的假支持）
    const aliases = propAliasesOf("context-menu");
    expect(aliases.map((alias) => alias.canonical)).toEqual(["items"]);
    expect(aliases[0]!.deprecated).toEqual(["menuItems"]);
  });

  it("emits ↔ 事件矩阵：open/close 来自矩阵，select 是显式登记的本库事件", () => {
    const sfc = BContextMenu as unknown as { emits?: string[] };
    // SDK 事件面 == 矩阵（双向）
  ...
  it("别名表：`menuItems` 已登记且只登记一次（防「收下但没人读」）", () => {
    const entries = OVERLAY_PROP_ALIASES.filter((alias) => alias.deprecated.includes("menuItems"));
    expect(entries).toHaveLength(1);
    expect(entries[0]!.kind).toBe("context-menu");
  });
});

 ...
 *
 * 覆盖边界（写清楚，别让它变成「看起来覆盖了」）：ADR 的私有面清单是
 * `_rd` / `qt=` / `getSeckeyAndSign`，这里只扫**可精确识别的两个名字**。
 * `qt=` 是 URL 查询参数名（`?qt=` / `&qt=`），消费方自备的代理 / 自托管地址可能合法地带上它，
 * 静态扫描会得到「误报逼人写豁免」的结果——而豁免列表一旦存在，就等于把门禁关掉了。它对应的
 * 真实风险（本库自建 UI Kit 的私有请求码）由「不自研标准 UI」这条 ADR 决策与评审把关，不在这里
 * 假装覆盖。另外：注释里要解释这类成员，用反引号（`` `_rd` ``）或换行写，别写成 `sdk._rd`——
 * 那与真的去读它无法区分，判违规是对的。
 */

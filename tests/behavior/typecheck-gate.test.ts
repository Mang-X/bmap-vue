 ...
 * 本次随补丁恢复（见 `upstream-types-case-patch.test.ts` 与 ADR 2026-09-13）。
 *
 ...
 * （`package` job 里也有 build，跨 job 取首个匹配会把顺序断言弄错）。
 *
 * 两条自我保护：
...
// 末尾的 `package` job 里也有 build：断言不能跨 job 取首个匹配。
const FIXTURE_TAIL = [
  "  package:",
  "    runs-on: ubuntu-latest",
  "    steps:",
...
const BUILD_STEP = [
  "      - name: Build artifacts for global bundle tests",
  "        run: node --experimental-strip-types scripts/build-package.mts",
];
...
describe("门禁判定自测（合成 workflow 负例）", () => {
  ...
  it("typecheck 排在 build 之后要报顺序问题", () => {
    const steps = [...BUILD_STEP, "      - name: Type-check package", `        run: ${TYPECHECK_COMMAND}`].join(
      "\n",
    );
    expect(problems(probeQualityGate(fixture(steps)))).toContain(
  ...
  it("typecheck 只出现在 package job 时要报「没有 step」（不能跨 job 取首个匹配）", () => {
    const qualitySteps = BUILD_STEP.join("\n");
    const onlyInPackageJob = fixture(qualitySteps).replace(
      "  package:\n    runs-on: ubuntu-latest\n    steps:\n      - name: Build package",
      [
        "  package:",
        "    runs-on: ubuntu-latest",
    ...
    expect(problems(probeQualityGate(onlyInPackageJob))).toContain(
      "没有 `run: pnpm typecheck:package` 这个 step",
    );
  });
});

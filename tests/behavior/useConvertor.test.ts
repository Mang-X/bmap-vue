...
import { useConvertor, CoordinatesFromType, CoordinatesToType } from '../../packages/bmap-vue/src/composables/useConvertor'
...
function mountChild(run: (c: ReturnType<typeof useConvertor>) => any) {
  const el = host()
  const collect = ref<any>(null)
  const Child = defineComponent({
    setup() {
      const conv = useConvertor()
      const exec = () => run(conv)
      onMounted(async () => {
        await exec()
      })

...
- **卸载时仍在飞的插件不再「复活」记录**：`PluginRegistry.dispose()` 之后晚到的结算被丢弃
  ...
  执行一次**（`map` 归注册表、`global` 归宿主）；`setup` 抛错时由所有者释放已经创建的实例
  （注册表与宿主两侧都如此，不会泄漏）；`PluginHost` 用纪元号隔离「上一轮 dispose 之前的在飞任务」，
  不会误删同名的新条目，旧纪元迟到的成功会**挂起**到这个名字有确定结论时再按 identity 判定释放
  （候选按实例去重，同一个实例不会被 dispose 两次）。此前一次卸载之后才下载完的插件会把状态从
  `disposed` 改回 `ready` 并往已销毁的地图上发事件。

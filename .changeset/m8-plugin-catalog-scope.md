...
- **卸载时仍在飞的插件不再「复活」记录**：`PluginRegistry.dispose()` 之后晚到的结算被丢弃
  （不改状态、不广播事件），`map` 作用域那条晚到的实例会被**就地释放**；`setup` 也只在「这次加载还算
  数」校验通过后才执行（地图销毁后再下载完的插件不会再产生 `setup` 副作用；`setup` 抛错的实例会被释放，
  不会泄漏）；`PluginHost` 用纪元号隔离「上一轮 dispose 之前的在飞任务」，不会误删同名的新条目，
  旧纪元迟到的成功会**挂起**到这个名字有确定结论时再按 identity 判定释放。此前一次卸载之后才下载完的
  插件会把状态从 `disposed` 改回 `ready` 并往已销毁的地图上发事件。

...
- **卸载时仍在飞的插件不再「复活」记录**：`PluginRegistry.dispose()` 之后晚到的结算被丢弃
  （不改状态、不广播事件），`map` 作用域那条晚到的实例会被**就地释放**；`setup` 只在「这次加载还算
  数」校验通过后才执行（地图销毁后再下载完的插件不会再产生 `setup` 副作用），且**只由资源的所有者
  执行一次**（`map` 归注册表、`global` 归宿主）；`setup` 前后各校验一次（`setup` 内同步调用 `dispose()`
  也不会把记录写回 `ready` 或让实例失去归属）；`setup` 抛错时由所有者释放已经创建的实例
  （注册表与宿主两侧都如此，不会泄漏，也不会与已挂起的候选撞成双重释放，更不会拆掉新纪元将要接管的
  同一个实例）；`PluginHost` 用纪元号隔离
  「上一轮 dispose 之前的在飞任务」，不会误删同名的新条目，旧纪元迟到的成功会**挂起**到这个名字有确定
  结论时再按 identity 判定释放（候选按实例去重，同一个实例不会被 dispose 两次）。此前一次卸载之后才
  下载完的插件会把状态从 `disposed` 改回 `ready` 并往已销毁的地图上发事件。

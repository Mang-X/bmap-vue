...
**公共扩展契约不变（评审 2026-09-22 P1 的修正）**：`urlPluginDefinition` 从根入口与 `./plugins`
...
**公共工厂的语义与本版之前一字不差：不设超时**（调用方仍可 `{ required: true }` 并自己决定等多久；
...
**行为变更（仅两处，都可预期）**：① 内置插件从「永远 `loading`」变成「60s 后 `error`」；
② 内置插件在超时 / 取消之后不再残留脚本元素。既有的 `error` 事件 / 「脚本加载成功但没暴露全局」/
`fetch` 失败三条路径的元素处理**未改**（元素仍留在文档里），以便本票的行为增量可核对。

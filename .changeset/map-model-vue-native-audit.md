...
- **`isControlled` 改为惰性创建**（本票唯一一处真正的 runtime 收口）。它库内零消费者，公共返回
  类型 `ControllableState.isControlled: ComputedRef<boolean>` **一字未改**（公共消费者照旧
  `state.isControlled.value`），但 `get` 取用时才建 ⇒ **`<Map>` 这条路径不再为它分配对象**。
  起因是复审八轮指出：早先「公共 API 保留该成员 ⇒ `<Map>` 必须继续实例化这份 runtime」是两件
  被混成一件的事。用例直接数 `computed` 的**分配次数**（`computed` 不计入 effect 计数，effect gate
  看不见这一类对象）——只断言缓存命中时 eager 版照样全过，改回 eager 实测变红。

...
- **三处没人用的内部对象 / 监听改为惰性或按条件创建**（本票仅有的 runtime 收口）：
  ...
  - **`defaultValue` 的 dev 告警 watcher 改为按条件注册**（`defaultValue && warn && isDev()`）。
    ...
    的 `!isDev()` 短路放在**分配 Set 之前**。标记仍交给消费方折叠，未在发布构建里定死。
  三条的用例都**直接数分配 / 数 effect**，不数行为（`computed` 数不出来、「无告警输出」也数不
  出来——production 下 eager 版同样一条不打印）：只断言「缓存命中」「告警次数」时 eager 版照样
  全过。实测：改回 eager / 去掉任一条件，三条 gate 各自变红。

...
`Marker` 因此删掉了 9 个手写 watcher 与全部生命周期代码；每个公开属性的更新策略（`position` /
...
- 新增**有界图标缓存**（同一个 Client / `<BMapProvider>` 一份，上限 200 条）：相同图标配置只构造一次 `BMap.Icon`，
  ...
  `driver.overlays.buildIcon()`（`useMarkerIcons()` 用的就是它）**每次调用仍新建实例**，

...
- **归属靠实例身份**：四个服务的回包没有请求身份、官方也没承诺跨请求回调顺序，因此同一实例同一时刻
  ...
  「最新者胜」，旧的迟到回包不会污染新结果。
- **状态码口径**：官方 `getStatus()` 声明的是 `ServiceStatus`（唯一表示成功的是 `0`），本库
  **`≥ 2` 即 `failed`** 并带上官方那个码；`0` / `1` / 读不到时由载荷决定 `success` / `empty`。
  （类型包里另有一套 `RouteStatus(0/1/2)`，但真实读数（无法规划 ⇒ `5`）证明运行时用的是
  `ServiceStatus`——方法签名优先。）
- 官方声明了但不生效的选项**不**暴露：`alternatives`（仅非 GL 模式）、`selectFirstResult`（仅

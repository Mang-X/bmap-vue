...
`Marker` 相同的声明式覆盖物内核，并建立**覆盖物事件矩阵**与**集中弃用层**。
...
- **新增 `Rectangle`**：v4 的矩形覆盖物，`bounds`（西南 / 东北）定义几何，与 Circle / Polygon / Polyline
...
- **大数组按根引用 + 版本 prop 更新**：`path` / `controlPoints` 不再做内容指纹（O(n) 序列化），
  ...
  把这条口径变成声明的一部分。
- **`GroundOverlay` 用 `bounds` 取代 `startPoint` + `endPoint`**：与上游
...
- **集中弃用层**（`core/deprecations`）：旧 prop / 旧事件名的身份（稳定 code + 替代 API）与
  「同实例只警告一次、production 不输出」的告警器；`Marker` 的历史事件名 `drag-end` 从组件里的
  硬编码收进这张表（组件不再出现旧名字）。
- **`OverlaySpec` 扩展**：新增 `kind`（事件矩阵与弃用表的查询键）、`watchSources`（字段 watch 源）、
  `fieldValues`（惰性值投影，`GroundOverlay.url` 的工厂在这里求值）、`afterMount`（创建完成后的
...
- 文档：新增「覆盖物事件矩阵」页（由用例逐条校验为矩阵的镜像）与 `Rectangle` 文档；
  `InfoWindow` / `ContextMenu` / `MapMask` / `Marker3D` 本次**不迁移**，
...
- `visible=false` 从 `removeOverlay` 改为 `hide()`：实例留在图上、只是不可见（与 `Marker`、控件
  从 #30 / #41 起的统一口径一致）。需要真正摘除请用 `v-if`；
- `remove` 事件只在**外部**摘除（`map.removeOverlay()` / `clearOverlays()`）时到达组件：组件自身的
  卸载 / 重建 / 隐藏不再回放它（此前 `<BezierCurve>` / `<Marker>` 在这些路径上会发）。

两处类型 / 元数据层的附带变化：`LabelStyle` 由 `Record<string, any>` 收紧为
`Record<string, unknown>`（`<Label style>` 的取值需自行收窄）；九个覆盖物组件统一补上

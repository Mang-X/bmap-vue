...
**`<CustomOverlay>`（新组件）**：用 Vue 内容做地图上的 DOM 覆盖物。
...
**`<ContextMenu>`**：两种写法，一份条目。

- 数据 API 的正典是 `items`（v3 的 `menuItems` **仍可用**，会在控制台提示一次；`items` 有值时旧名完全不参与）。
- 新增声明式 API `<MenuItem>` / `<MenuSeparator>`：与数据 API 归一化成同一份条目（顺序、`disabled`、
...
- 写在 `<Marker>` 里的 `<ContextMenu>` 现在挂在**该标注**上（`Marker#addContextMenu`）；
  写在 `<Map>` 下则挂到地图（`Map#addContextMenu`）。

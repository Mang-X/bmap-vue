...
- `<Map>` 对传入的 `center` / `defaultCenter` **做防御性拷贝**：父级原地修改自己的点对象不会污染内部状态、`default*` 初值与 `resetView()` 的首次快照（请仍按 Vue 常规语义「换引用」更新 props）。
- 新增公开 composable `useControllableState`（`<Map>` 内部用的就是它的三态规则），可复用于业务侧自建受控字段；传可变对象时用 `copy` 选项声明拷贝方式。

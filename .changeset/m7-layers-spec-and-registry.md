...
- **永久销毁（卸载 / 重建 / Map 销毁）保证最终清空**数据驱动图层（`clearData` →
  `removeAllOverlays`，即 `DOMLayer` 的 `removeAllOverlays()`）。但如果 `visible=false` 已经先成功
  摘过一次，永久销毁做的就只是 **detached cleanup**（清空 + 结束），**不会**再调一次 `removeLayer`
  ——官方没有承诺「对已经摘掉的图层重复 `removeLayer` 是安全的」，本库不猜。临时摘挂
...
- **挂载状态是三态（`attached` / `detached` / `unknown`）**：`addLayer` / `removeLayer` 抛错之后
  ...
  图上的实例被挂第二份。未知状态由「先 best-effort 摘一次、再挂」这个确定性动作收敛。

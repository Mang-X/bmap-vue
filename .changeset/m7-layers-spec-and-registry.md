...
- **永久销毁（卸载 / 重建 / Map 销毁）的清理按「清空操作的作用域」分述**（不再是笼统的「保证最终
  清空」）：覆盖物在永久销毁后一定不在图上——`GeoJSONLayer` 的 `clearData()` 是 **Map 作用域**
  （官方明说「要真正清空 `getData()` 集合得在 `removeLayer` **之前**调」），因此只在图层仍在图上时
  调用；已 detached 时跳过它（对已摘下的实例调用没有效果），可见资源由 `removeLayer` 自己摘掉。
  `DOMLayer.removeAllOverlays()` 是**图层作用域**（移除图层自己创建的真实 DOM 节点），与是否挂图
  无关，照常执行。`getData()` 集合在 detached 路径上不会被清——它是随实例一起丢弃的内存状态，
  不是需要释放的 SDK 资源。
...
- **挂载状态是三态（`attached` / `detached` / `unknown`）**：`addLayer` / `removeLayer` 抛错之后
  ...
  ⚠️ 这一步依赖一条**未经上游证明**的前提：对已经摘掉的图层重复 `removeLayer` 是安全的。本库不假装
  它已被证明——前提不成立时收敛**不会发生**（图层停在未知、仍不在地图上），但失败可观测
  （`resource:error`）且**绝不会**出现两份；取证结论见 ADR 决策 12b / 已知限制 14。
